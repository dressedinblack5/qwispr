...
function hardwareList() {
  ...
  const qpuWarning = qpuStatus.backend === Backend.ibm && !qpuStatus.hasToken && !qpuStatus.dryRun ? "QISKIT_TOKEN missing — falling back to simulator" : qpuStatus.dryRun && qpuStatus.backend === Backend.ibm ? "dry-run: simulated QPU path" : undefined;
  console.log(JSON.stringify({ backends: rows, current, env: { QWISPR_BACKEND: envBackend, QWISPR_DEVICE: envDevice, deviceString: getDeviceString() }, qpuStatus, ...(qpuWarning ? { qpuWarning } : {}) }, null, 2));
...
function printHelp() {
  ...
  console.log("    qwispr run --task resolve --qubo <file> [--backend simulator|lightning|ibm|braket]");
  ...
  console.log("    qwispr run --task resolve|search|testgen|analyze|refactor [args...] [--backend ...]");
  ...
  console.log("");
  console.log("Env: QWISPR_BACKEND, QWISPR_DEVICE (e.g. lightning.qubit), QWISPR_LAYERS, QWISPR_ITERS, QWISPR_CALIBRATION");
}
...
async function main() {
  ...
  const alias: Record<string, string> = {
    "code-graph": "analyze", "code_graph": "analyze", "qwalk": "analyze",
    "grover": "search",
    "dep-agent": "resolve", "qaoa": "resolve",
  };
  if (alias[args[0]]) {
  ...
  if (args[0] === "vqe") {
    ...
    if (!quboFile) { console.error("usage: qwispr vqe --qubo <file> --layers 2 --iters 50"); process.exit(1); }
