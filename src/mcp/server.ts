...
const TOOLS = [
  {
    ...
    inputSchema: {
      type: "object",
      properties: { file: { type: "string", description: "Path to source file" }, entry: { type: "string", description: "Entry function name" } },
      required: ["file"],
    },
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { pattern: { type: "string" }, files: { type: "string", description: "Glob pattern" }, top: { type: "number" } },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { file: { type: "string" }, function: { type: "string", description: "Function name" }, layers: { type: "number" } },
...
async function dispatch(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case "analyze": return analyze({ file: args.file as string, entry: args.entry as string | undefined });
    case "search": return search({ pattern: args.pattern as string, files: args.files as string, top: args.top as number | undefined });
    case "testgen": return generateTestInputs({ file: args.file as string, functionName: (args.function ?? args.functionName) as string, layers: args.layers as number | undefined });
    case "refactor": return refactor({ file: args.file as string, top: args.top as number | undefined });
    case "hardware": {
      const current = getBackend();
      const rows = (Object.values(Backend) as string[]).map((b) => `${b} -> ${getDeviceString(b as Backend)}${b === current ? " (current)" : ""}`);
      return { backends: rows, current, env: { QWISPR_BACKEND: process.env.QWISPR_BACKEND ?? "(unset)", QWISPR_DEVICE: process.env.QWISPR_DEVICE ?? "(unset)", deviceString: getDeviceString() } };
    }
  ...
  const handle = async (msg: unknown) => {
    ...
    try {
      if (method === "initialize") {
        const res = { jsonrpc: "2.0", id, result: { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "qwispr", version: "0.1.0" } } };
        send(res);
