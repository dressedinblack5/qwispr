  ...
  for (let lineNum = 0; lineNum < lines.length; lineNum++) {
    ...
    while (col < line.length) {
      ...
      if (/[a-zA-Z_$]/.test(char)) {
        ...
        tokens.push({ 
          type: /^(const|let|var|function|class|if|else|for|while|return|import|export|from|async|await|try|catch|finally|throw|new|this|super|extends|implements|interface|type|enum|namespace|declare|abstract|private|protected|public|static|readonly|get|set)$/.test(value) ? "keyword" : "identifier", 
          value, start, end 
        });
        col = endCol;
        continue;
...
function parseSimple(source: string): ASTNode {
  ...
  while (i < tokens.length) {
    ...
    if (token.type === "keyword" && (token.value === "function" || token.value === "const" || token.value === "let" || token.value === "var")) {
      ...
      i = j + 1;
    } else if (token.type === "keyword" && (token.value === "if" || token.value === "for" || token.value === "while")) {
      const start = token.start;
      let end = token.end;
      let j = i + 1;
...
export function parse(source: string, filePath: string): ASTNode {
