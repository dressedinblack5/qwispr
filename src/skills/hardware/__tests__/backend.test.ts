...
afterEach(() => {
  if (origBackend === undefined) delete process.env.QWISPR_BACKEND;
  else process.env.QWISPR_BACKEND = origBackend;
  if (origDevice === undefined) delete process.env.QWISPR_DEVICE;
  else process.env.QWISPR_DEVICE = origDevice;
});
