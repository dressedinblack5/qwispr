...
describe("learning", () => {
  ...
  beforeEach(() => {
    tmpFile = path.join(os.tmpdir(), `qwispr-test-${Date.now()}-${Math.random().toString(36).slice(2)}.jsonl`);
  ...
  it("append/read works and opt-in off doesn't write", () => {
    ...
    appendEvent({ ts: Date.now(), task: "search", route: "classical", nVars: 2, wallMs: 10, success: true });
  ...
  it("quantum 3x slower without gain pushes threshold up", () => {
    ...
    for (let i = 0; i < 10; i++) {
      appendEvent({ ts: Date.now(), task: "resolve", route: "classical", nVars: 2, wallMs: 10, success: true });
      appendEvent({ ts: Date.now(), task: "resolve", route: "quantum", nVars: 10, wallMs: 30, success: true });
    }
