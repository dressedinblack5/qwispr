...
export interface RefactorCandidate { file: string; function: string; score: number; reason: string; centrality: number; cohesion: number; }
export interface RefactorResult { candidates: RefactorCandidate[]; }
...
export function refactorSource(source: string, file: string, top = 5): RefactorResult {
  ...
  const candidates: RefactorCandidate[] = nodes.map(n => {
    ...
    return { file, function: n, score: Number(score.toFixed(4)), reason, centrality: c, cohesion: coh };
  });
