...
function expandGlob(pattern: string): string[] {
  ...
  if (path.isAbsolute(pattern) && process.env.QWISPR_ALLOW_ABSOLUTE !== '1') {
    throw new Error(
      `qwispr: absolute glob rejected (set QWISPR_ALLOW_ABSOLUTE=1 to allow): ${pattern}`
    );
  }
  ...
  const basePart =
    globIdx === -1
      ...
      : pattern.slice(0, pattern.lastIndexOf('/', globIdx) + 1) || '.';
  ...
  try {
    ...
    if (isInsideRoot(baseResolved) && fs.existsSync(base) && fs.statSync(base).isDirectory())
      walkBase = base;
    else walkBase = root;
  } catch {
    walkBase = root;
...
export function search(opts: { pattern: string; files: string; top?: number }): SearchResult {
  ...
  try {
    ...
    throw new Error(`qwispr: invalid pattern: ${opts.pattern} — ${(e as Error).message}`);
  }
  ...
  for (const file of fileList) {
    ...
    lines.forEach((snippet, idx) => {
      if (regex.test(snippet))
        allHits.push({ file, line: idx + 1, snippet: snippet.trim().slice(0, 200) });
    });
  }
  ...
  if (hits.length > 0) {
    ...
    const probs = groverProbabilities(
      n,
      Array(n)
        .fill(true)
        .map((_, i) => i < hits.length),
      groverIterations(n, hits.length)
    );
    // probs for marked subset; pick max among hits
    const hitProbs = probs.slice(0, hits.length);
    let maxP = -1;
    hitProbs.forEach((p, i) => {
      if (p > maxP) {
        maxP = p;
        amplifiedIndex = i;
      }
