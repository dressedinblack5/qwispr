...
export interface TestgenResult { inputs: number[][]; coverageHint: string; }
...
function extractFunction(src: string, fn: string): { params: string[]; body: string } | null {
  ...
  try {
    ...
    patterns = [
      new RegExp(`(?:export\\s+)?(?:async\\s+)?function\\s+${esc}\\s*\\(([^)]*)\\)\\s*(?::\\s*[^{]+)?\\{`),
      new RegExp(`(?:export\\s+)?(?:const|let|var)\\s+${esc}\\s*=\\s*\\(([^)]*)\\)\\s*(?::\\s*[^{]+)?=>\\s*\\{`),
      new RegExp(`(?:export\\s+)?(?:const|let|var)\\s+${esc}\\s*=\\s*(?:async\\s+)?function\\s*\\(([^)]*)\\)\\s*(?::\\s*[^{]+)?\\{`),
    ];
  } catch {
  ...
  let arrow: RegExp;
  try {
    arrow = new RegExp(`(?:export\\s+)?(?:const|let|var)\\s+${esc}\\s*=\\s*\\(([^)]*)\\)\\s*(?::\\s*[^{]+)?=>\\s*([^;\\n]+)`);
  } catch {
    throw new Error(`qwispr: invalid function name "${fn}"`);
  }
...
function buildQubo(params: string[], body: string): number[][] {
  ...
  while ((m = ifRe.exec(body))) {
    ...
    if (/^!\w+$/.test(cond)) {
      ...
      if(paramIdx.has(v)){ const pi=paramIdx.get(v)!; for(let k=0;k<BITS;k++) Q[pi*BITS+k][pi*BITS+k] += (1<<k); }
      continue;
    }
    ...
    if (leftIsParam && rightIsNum) {
      ...
      else if (op==="==="||op==="==") { /* minimize |x-C|: push toward C via quadratic would be ideal, naive linear toward C */ 
      ...
      if (op===">"||op===">=") { addLinear(left,-1); addLinear(right,1); }
      else if (op==="<"||op==="<=") { addLinear(left,1); addLinear(right,-1); }
    }
  }
...
export async function generateTestInputs(opts: { file: string; functionName: string; layers?: number }): Promise<TestgenResult> {
  ...
  const triggersBranch = (vals: number[]) => {
    ...
    while ((mm = re.exec(body))) {
      ...
      try {
        let expr = cond;
        for (const [k, v] of Object.entries(env)) expr = expr.replace(new RegExp(`\\b${k}\\b`, "g"), String(v));
